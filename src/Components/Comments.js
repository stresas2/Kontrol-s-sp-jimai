...
class Comments extends Component {
  ...
  openMap = stotele => {
    this.setState({
      komentarai: this.state.komentarai,
      loading: this.state.loading,
      model: {
        showModal: true,
        name: stotele.tagas,
        Koordinates: stotele.Koordinates
      },
  ...
  closeMap = () => {
    this.setState({
      ...
      model: {
        ...
        Koordinates: this.state.model.Koordinates
      },
      model2: this.state.model2
    });
  };
  ...
  openComment = stotele => {
    this.setState({
      ...
      loading: this.state.loading,
      model: {
        showModal: this.state.model.showModal,
        name: stotele.tagas,
        Koordinates: stotele.Koordinates
      },
      model2: true
  ...
  increase = vardas => {
    ...
    if (!cookies.get(vardas._id)) {
      var index = this.state.komentarai.findIndex(
        emp => emp._id === vardas._id
      );
      vardas.like += 1;
      const updatedLikes = update(this.state.komentarai, {
        $splice: [[index, 1, vardas]]
      ...
      restdb
        .put("/rest/komentarai/" + vardas._id, vardas)
  ...
  render() {
    ...
    var mapas = (
      ...
      >
        ...
        <Map2
          vardas={this.state.model.name}
          koordinates={this.state.model.Koordinates}
        />
        <div className="text-center">
          <p className="text-dark h6 pt-2" style={{ lineHeight: 1.5 }}>
    ...
    listItems = this.state.komentarai.map(vardas => (
      <div
        className="container-fluid bg-white mb-4 fade-in p-0 shadow"
        key={vardas._id}
      ...
      >
        ...
        <div
          className="container-fluid  text-dark text-justify p-3 text-wrap"
          style={{ minHeight: 80 }}
        >
          {vardas.komentaras}
        </div>
        ...
        <div className="d-flex">
          <div className="p-2 ml-2">
            <i
              className="fas fa-thumbs-up pr-2"
              id="like"
              onClick={this.increase.bind(this, vardas)}
              style={{ fontSize: "20px" }}
            ></i>
            ...
            <i
              className="fas fa-thumbs-down  pl-2"
              id="dislike"
              onClick={this.decrease.bind(this, vardas)}
              style={{ fontSize: "20px" }}
            ></i>
          </div>
          ...
          <div className="ml-auto">
            <button
              className="btn btn-outline-warning m-1 btn-sm"
              onClick={this.openMap.bind(this, vardas)}
            >
              {vardas.tagas}
            </button>

            <i
              className="fas fa-map-marker-alt m-2"
              style={{ fontSize: "20px", color: "#87acb5" }}
            ></i>
          </div>
        </div>
      </div>
    ...
    const emty = (
      <div
        className="container-fluid bg-white mb-4 fade-in p-0 shadow"
        style={{ borderRadius: "10px", height: "60px", position: "relative" }}
      >
    ...
    const mobileButton = (
      <div
        className="d-block d-md-none fixed-bottom mb-3 dot bg-light"
