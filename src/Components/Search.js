      ...
      <div className="px-2 pb-1">
          ...
          <div className="input-group input-group-sm border-bottom">
            <div className="input-group-prepend">
              <div className="input-group-text border-0 rounded-0"  style={{backgroundColor: "#87acb5"}}><i className="fa fa-search" style={{color: "#fff", fontSize: "20px"}}></i></div>
            </div>
            <input type="text" className="form-control border-0 rounded-0"  id="myInput" placeholder="Stotelės paieška..." onChange = {this.stotelesFilter}  />
          </div>

              <ul className="d-block text-dark" id="myUL">
                {stoteles}
              </ul>
          </div>

            { this.state.tagas ? <button className="btn btn-primary btn-sm mt-1" onClick={this.onDelete}> {this.state.tagas} <span aria-hidden="true">&times;</span></button>  : null  }
