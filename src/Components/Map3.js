...
import { Map, TileLayer, Marker, Popup, CircleMarker, Tooltip, Pane, Rectangle } from 'react-leaflet';
...
export default class Map3 extends Component {
  ...
  newPost = (data) => {
    ...
    if(das.hasOwnProperty(data.tagas)){
      ...
      das[data.tagas].unshift({data: data.data, koordinate: data.Koordinates});
  ...
  pranesimuSort = (komentai) => {
      ...
      const pranesimai = items.reduce((catMemo, { tagas, data, Koordinates }) => {
       (catMemo[tagas] = catMemo[tagas] || []).push({data: data, koordinate: Koordinates});
       return catMemo;
       }, {});
  ...
  render() {
    ...
    var listItems;
      ...
      loading =
      <div className="h-100 d-flex justify-content-center align-items-center"><img src="/css/loader2.gif" alt="" /></div>;
    ...
    iconSize: [32,32],
      ...
      listItems = Object.keys(this.state.komentarai).map((vardas) =>
        <Marker position={koordinate(this.state.komentarai[vardas][0].koordinate)} icon={ iconPerson }>
          <Popup>
            {vardas} <br />
    ...
    return (
      ...
      <Map center={vilnius} zoom={12} zoomControl={true} viewport={vilnius} style={{width: "100%", height: "100%", zIndex: "0"}}>
        <MapInfo />
